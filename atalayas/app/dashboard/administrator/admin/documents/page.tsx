...
export default function DocumentsExplorerPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) { 
            // CAMBIO: console.warn en lugar de console.error para evitar el overlay en dev
  ...
  const filteredDocs = documents.filter(doc => {
    if (currentUser?.role === 'ADMIN') {
      if (!doc.isPublic && String(doc.companyId) !== String(currentUser.companyId)) return false;
    }
    if (filter === 'public'  && !doc.isPublic)  return false;
    if (filter === 'private' &&  doc.isPublic)  return false;
    if (searchTerm.trim()) {
      const norm = (s: string) => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
      return norm(doc.title).includes(norm(searchTerm.trim()));
    }
    return true;
  });
  ...
  return (
    <motion.div className="flex h-screen bg-background overflow-hidden font-sans text-foreground"
          ...
          <PageHeader
            ...
            action={
              <Link href="/dashboard/administrator/admin/documents/new">
                <button className="bg-secondary text-secondary-foreground px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-wider hover:opacity-90 transition-all flex items-center gap-2 shadow-sm">
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          >
            ...
            <div className="absolute inset-0 overflow-hidden">
              <div className="w-[320px] h-full bg-card border-r border-border flex flex-col shadow-2xl lg:shadow-none">
                ...
                {/* Búsqueda + filtro */}
                <motion.div className="p-4 sm:p-6 border-b border-border/50 relative z-50"
                  variants={panelVariants} initial="hidden" animate="show">
                        ...
                        <div className="flex flex-col gap-1">
                          {[
                            ...
                            <motion.button key={opt.id} whileHover={{ x: 3 }} whileTap={{ scale: 0.98 }}
                              ...
                              className={`flex items-center gap-3 w-full px-4 py-2 rounded-xl text-xs font-bold transition-all
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar bg-card">
                  {loading ? <LoadingSpinner /> : filteredDocs.map(doc => (
                    <motion.button key={doc.id}
                      initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                      whileHover={{ x: 4, scale: 1.01 }} whileTap={{ scale: 0.99 }}
                      onClick={() => { setSelectedDoc(doc); if (!isDesktop) setIsSidebarOpen(false); }}
                      className={`w-full group flex items-start gap-3 p-4 rounded-2xl transition-all border
            ...
            <motion.button
              initial={false}
              animate={{ left: isSidebarOpen ? SIDEBAR_W : 0 }}
              transition={spring}
              whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
              onClick={() => setIsSidebarOpen(v => !v)}
              className="absolute top-1/2 -translate-y-1/2 w-6 h-12 bg-card border-y border-r border-border
                         flex items-center justify-center rounded-r-lg shadow-xl hover:bg-muted transition-colors z-50">
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-background">
            <AnimatePresence mode="wait">
              {selectedDoc ? (
                <motion.div key={selectedDoc.id} className="flex flex-col h-full"
                  ...
                  <div className="px-4 sm:px-8 py-4 bg-card/50 border-b border-border flex flex-col sm:flex-row items-start sm:items-center justify-between backdrop-blur-sm z-10 gap-4 shrink-0">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                      <motion.button whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }}
                        ...
                        className="px-4 py-2 bg-muted hover:bg-primary hover:text-white rounded-xl text-xs font-normal transition-all flex items-center gap-2">
                        <i className="bi bi-download" /> Descargar
                      </motion.button>
                      <motion.a whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }}
                        href={selectedDoc.fileUrl} target="_blank" rel="noopener noreferrer"
                        className="px-4 py-2 bg-muted hover:bg-primary hover:text-white rounded-xl text-xs font-normal transition-all flex items-center gap-2">
                  ...
                  <div className="flex-1 overflow-hidden bg-[#525659]">
                    <iframe src={`${selectedDoc.fileUrl}#toolbar=0`}
                ...
                <motion.div key="empty" className="flex-1 flex flex-col items-center justify-center p-12 text-center"
