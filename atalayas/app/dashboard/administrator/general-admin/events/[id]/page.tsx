...
export default function GAdminEventDetailPage() {
  const params = useParams();
  const router = useRouter();
  const [event, setEvent] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [isRsvping, setIsRsvping] = useState(false);
  const [companies, setCompanies] = useState<any[]>([]); // Lista de empresas para el selector
    ...
    try {
      ...
      setEditForm({
        ...
        event_date: data.event_date
          ...
          : "",
        companyId: data.companyId || "public",
      });
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleDownloadCSV = () => {
    ...
    const headers = ["Nombre", "Email", "Empresa", "Estado"];
    const rows = confirmedAttendees.map((rsvp: any) => [
      rsvp.User?.name || "N/A",
      rsvp.User?.email || "N/A",
      rsvp.User?.Company?.name,
      "Confirmado"
    ]);
    const csvContent = ["sep=,", headers.join(","), ...rows.map((e: string[]) => e.join(","))].join("\n");
  ...
  const handleRSVP = async (status: boolean) => {
    ...
    if (index !== -1) newAttendees[index] = { ...newAttendees[index], status };
    else newAttendees.push({ user_id: user.id, status, User: user });
  ...
  const handleDelete = async () => {
    try {
      ...
      if (res.ok) router.push("/dashboard/administrator/general-admin/events");
    } catch (err) {
      alert("Error al eliminar");
    }
  };

  if (loading || !event) return <div className="flex h-screen items-center justify-center bg-[#f5f5f7] dark:bg-[#0d0d0f]"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-primary" /></div>;
      ...
      <main className="flex-1 overflow-auto flex flex-col no-scrollbar">
        <PageHeader
          ...
          action={
            <button onClick={() => setIsEditModalOpen(true)} className="p-3 px-6 rounded-2xl bg-primary text-white text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-lg cursor-pointer">
              <i className="bi bi-pencil-fill mr-2"></i> Editar el evento
            </button>
          }
        />
        ...
        <div className="p-6 lg:p-10 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-[#1c1c1e] rounded-[3rem] p-8 lg:p-12 border border-black/5 shadow-sm">
              {event.image_url && (
              ...
              <h3 className="text-xl font-black mb-4 uppercase tracking-tighter italic">Descripción</h3>
              <p className="text-zinc-500 dark:text-zinc-400 text-lg leading-relaxed whitespace-pre-wrap">{event.description || "Sin descripción disponible."}</p>
                  ...
                  {event.EventAttendees?.filter((a: any) => a.status === true).map((rsvp: any) => (
                    <div key={rsvp.user_id} className="flex items-center gap-3 p-3 rounded-2xl bg-zinc-50 dark:bg-white/5 border border-black/5">
                      <div className="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center text-[10px] font-black uppercase overflow-hidden">
                        {rsvp.User?.avatarUrl ? <img src={rsvp.User.avatarUrl} className="w-full h-full object-cover" /> : <span>{rsvp.User?.name?.charAt(0)}</span>}
                      </div>
          ...
          <aside className="lg:col-span-4 space-y-6">
            <div className="sticky top-6 space-y-4">
              <div className="bg-white dark:bg-[#1c1c1e] rounded-[2.5rem] p-8 border border-black/5 shadow-sm">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-6 text-center italic">Logística y RSVP</h4>
                <div className="space-y-4 mb-8">
                  <div className="flex items-center gap-3 p-4 bg-zinc-50 dark:bg-white/5 rounded-2xl">
                    <i className="bi bi-calendar3 text-primary"></i>
                    <span className="text-xs font-bold">{eventDate.toLocaleDateString("es-ES", { day: "2-digit", month: "long", year: "numeric" })}</span>
                  </div>
                   <div className="flex items-center gap-3 p-4 bg-zinc-50 dark:bg-white/5 rounded-2xl">
              ...
              <button onClick={handleDownloadCSV} className="w-full py-5 bg-white dark:bg-[#1c1c1e] rounded-[2rem] border border-black/5 flex items-center justify-center gap-3 hover:bg-gray-50 transition-all group shadow-sm cursor-pointer">
              ...
              <button onClick={() => setIsDeleteModalOpen(true)} className="w-full py-4 text-red-500 text-[10px] font-black uppercase tracking-widest hover:underline transition-all cursor-pointer">
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="bg-white dark:bg-[#1c1c1e] w-full max-w-2xl rounded-[3rem] p-10 overflow-y-auto max-h-[90vh] shadow-2xl border border-white/10 no-scrollbar">
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setIsEditModalOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-full bg-zinc-100 dark:bg-white/5"><i className="bi bi-x-lg"></i></button>
              </div>
              <form onSubmit={handleUpdate} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="text-[10px] font-black uppercase ml-1 opacity-60 italic">Título</label>
                    <input type="text" value={editForm.title ?? ""} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })} className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none outline-none font-bold focus:ring-2 ring-primary/20" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase ml-1 opacity-60 italic">Fecha y Hora</label>
                    <input type="datetime-local" value={editForm.event_date ?? ""} onChange={(e) => setEditForm({ ...editForm, event_date: e.target.value })} className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none outline-none font-bold focus:ring-2 ring-primary/20" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase ml-1 opacity-60 italic">Aforo Máx.</label>
                    <input type="number" value={editForm.max_capacity ?? ""} onChange={(e) => setEditForm({ ...editForm, max_capacity: e.target.value })} className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none outline-none font-bold focus:ring-2 ring-primary/20" />
                  </div>
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase ml-1 opacity-60 italic">Ubicación</label>
                  <input type="text" value={editForm.location ?? ""} onChange={(e) => setEditForm({ ...editForm, location: e.target.value })} className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none outline-none font-bold focus:ring-2 ring-primary/20" />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase ml-1 opacity-60 italic">Imagen</label>
                  <input type="file" accept="image/*" onChange={(e) => setSelectedFile(e.target.files?.[0] || null)} className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none outline-none font-bold text-xs" />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase ml-1 opacity-60 italic">Descripción</label>
                  <textarea rows={4} value={editForm.description ?? ""} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} className="w-full p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-none outline-none font-bold resize-none focus:ring-2 ring-primary/20" />
                </div>
                <div className="pt-6 border-t border-black/5 flex flex-col gap-3">
                  <button type="submit" disabled={editLoading} className="w-full py-5 bg-primary text-white rounded-2xl font-black uppercase tracking-widest shadow-xl hover:scale-[1.02] transition-all disabled:opacity-50">{editLoading ? "Sincronizando..." : "Actualizar información"}</button>
                  <button type="button" onClick={() => setIsEditModalOpen(false)} className="w-full py-4 text-[10px] font-black uppercase opacity-40">Cancelar</button>
                </div>
              </form>
            </motion.div>
          </div>
        ...
        {isDeleteModalOpen && (
          <div className="fixed inset-0 z-110 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white dark:bg-[#1c1c1e] p-10 rounded-[3rem] max-w-sm text-center shadow-2xl">
              ...
              <p className="text-xs text-muted-foreground mb-8">Como General Admin, esta acción eliminará el evento permanentemente para todos.</p>
